...
// ─── CORS (dynamic origin checker with logging) ────────────
const corsOriginsRaw = process.env.CORS_ORIGINS || `${FRONTEND_URL},https://deepgen-ai-1.onrender.com,http://localhost:3000`;
const allowedOrigins = corsOriginsRaw.split(',').map(o => o.trim());

// If the list contains "*", allow all origins (for testing only)
...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      if (allowAll) {
        console.log(`CORS: Allowing all origins (wildcard) - origin: ${origin}`);
